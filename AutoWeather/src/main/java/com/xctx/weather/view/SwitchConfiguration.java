...
/**
 * class for configuring the Switchbutton used when you want to change the face
 * of this view in code
 * 
 */
public class SwitchConfiguration implements Cloneable {
	...
	private int mOnColor = Default.DEFAULT_ON_COLOR,
			mOffColor = Default.DEFAULT_OFF_COLOR,
			mThumbColor = Default.DEFAULT_THUMB_COLOR,
			mThumbPressedColor = Default.DEFAULT_THUMB_PRESSED_COLOR;

	/**
	 * space between View's border and thumb
	 */
	private int mThumbMarginTop = 0, mThumbMarginBottom = 0,
			mThumbMarginLeft = 0, mThumbMarginRight = 0;
	...
	public static SwitchConfiguration getDefault(float density) {
		SwitchConfiguration defaultConfiguration = new SwitchConfiguration();
		defaultConfiguration.density = density;
		defaultConfiguration.setThumbMarginInPixel(defaultConfiguration
				.getDefaultThumbMarginInPixel());
	 ...
	 * 
	 * @param velocity
	 *            pixel moved per frame (in linear)
	 */
	...
	public Drawable getThumbDrawableWithFix() {
		if (mThumbDrawable != null) {
			...
			int[] stateSet = null;
			try {
				Field stateField = View.class
						.getDeclaredField("PRESSED_ENABLED_STATE_SET");
