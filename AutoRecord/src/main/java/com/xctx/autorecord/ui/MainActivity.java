package com.xctx.autorecord.ui;

import java.io.File;
import java.text.SimpleDateFormat;
import java.util.Date;

import com.xctx.autorecord.Constant;
import com.xctx.autorecord.MyApp;
import com.xctx.autorecord.R;
import com.xctx.autorecord.Typefaces;
...
import com.xctx.autorecord.service.SensorWatchService;
import com.xctx.autorecord.thread.MoveImageThread;
import com.xctx.autorecord.thread.WriteImageExifThread;
import com.xctx.autorecord.util.ClickUtil;
import com.xctx.autorecord.util.DateUtil;
import com.xctx.autorecord.util.FileUtil;
import com.xctx.autorecord.util.Flash2SDUtil;
import com.xctx.autorecord.util.HintUtil;
import com.xctx.autorecord.util.MyLog;
import com.xctx.autorecord.util.ProviderUtil;
import com.xctx.autorecord.util.ProviderUtil.Name;
import com.xctx.autorecord.util.SettingUtil;
import com.xctx.autorecord.util.StorageUtil;
import com.xctx.autorecord.view.BackLineView;
import com.xctx.tachograph.TachographCallback;
...
//	private TachographRecorder recorderFront;
	private MediaRecorder recorderFront;
	private int intervalState, muteState;
	int isFrontRecording = 0;
...
//	private TachographRecorder recorderBack;
	private MediaRecorder recorderBack;
	int isBackRecording = 0;
	// 倒车线控制
	...
	public void takePhoto(boolean playSound) {
		if (!MyApp.isFrontRecording && !MyApp.isBackRecording
			...
			HintUtil.playAudio(getApplicationContext(),
					com.xctx.tachograph.TachographCallback.FILE_TYPE_IMAGE,
					playSound);
						...
						mMainHandler.post(new Runnable() {
							...
							public void run() {
								...
								HintUtil.playAudio(
										...
										com.xctx.tachograph.TachographCallback.FILE_TYPE_VIDEO,
										MyApp.isAccOn);
...
//			recorderBack.setVideoSeconds(seconds);
		...
		//return (recorderFront != null) ? recorderFront.setVideoSeconds(seconds) : -1;
		return -1;
	}

	/** 设置视频重叠 */
	public int setFrontOverlap(int seconds) {
		//return (recorderFront != null) ? recorderFront.setVideoOverlap(seconds) : -1;
		return -1;
	}

	/** 设置视频重叠 */
	public int setBackOverlap(int seconds) {
		//return (recorderBack != null) ? recorderBack.setVideoOverlap(seconds) : -1;
		return -1;
	}
...
//			recorderFront = new TachographRecorder();
			recorderFront = new MediaRecorder();
...
/*			recorderFront.setTachographCallback(new FrontTachographCallback());
			...
			recorderFront.reset();
			recorderFront.setCamera(cameraFront);
			recorderFront.setAudioSource(MediaRecorder.AudioSource.DEFAULT);
			recorderFront.setVideoSource(MediaRecorder.VideoSource.CAMERA);
			CamcorderProfile mCamcorderProfile = CamcorderProfile.get(Camera.CameraInfo.CAMERA_FACING_BACK,
					CamcorderProfile.QUALITY_HIGH);
			recorderFront.setProfile(mCamcorderProfile);
			recorderFront.setOutputFile(getOutputMediaFile(MEDIA_TYPE_VIDEO).toString());
			recorderFront.setPreviewDisplay(surfaceHolderFront.getSurface());
	...
	private File getOutputMediaFile(int type) {
		...
		if (! mediaStorageDir.exists()){
			if (! mediaStorageDir.mkdirs()){
				MyLog.v("failed to create directory");
				return null;
			}
		}
	...
	private void setupBackRecorder() {
		...
		try {
			...
			recorderBack = new MediaRecorder();
			recorderBack.reset();
			recorderBack.setCamera(cameraFront);
			recorderBack.setAudioSource(MediaRecorder.AudioSource.DEFAULT);
			recorderBack.setVideoSource(MediaRecorder.VideoSource.CAMERA);
			CamcorderProfile mCamcorderProfile = CamcorderProfile.get(Camera.CameraInfo.CAMERA_FACING_BACK,
					CamcorderProfile.QUALITY_HIGH);
			recorderBack.setProfile(mCamcorderProfile);
			recorderBack.setOutputFile(getOutputMediaFile(MEDIA_TYPE_VIDEO).toString());
			recorderBack.setPreviewDisplay(surfaceHolderFront.getSurface());
...
//				recorderBack.close();
		...
		public void onFileSave(int type, String path) {
			try {
				if (type == 1) { // 视频
					...
					Flash2SDUtil.moveVideoToSD(context, false, 1 == videoLock,
							videoName);
				...
				sendBroadcast(new Intent(Intent.ACTION_MEDIA_SCANNER_SCAN_FILE,
						Uri.parse("file://"
								+ (Constant.Record.flashToCard ? path.replace(
										"sdcard0", "sdcard1") : path)))); // 更新Media
																			// Database
				MyLog.d("Back.onFileSave.Type="
						+ type
						+ ",Save path:"
						+ (Constant.Record.flashToCard ? path.replace(
								"sdcard0", "sdcard1") : path));
	...
	public boolean isFrontRecord() {
		if (recorderFront != null) {
			int intFrontRecording = isFrontRecording;
	...
	public boolean isBackRecord() {
		if (recorderBack != null) {
			int intBackRecording = isBackRecording;
